...
from src.datasets import ImageNet, ImageNetteDataModule
from src.models import ImageNetteResNet
...
def parse_args():
    ...
    # basic
    parser.add_argument('--load', type=str, default='clean')
    ...
    # dataset
    parser.add_argument('--data-dir', type=Path, default='static/datasets/imagenette2-320')
    parser.add_argument('--data-skip', type=int, default=5)
    # attack
    ...
    # auto pgd
    parser.add_argument('-a', '--attack', type=str, default='pgd', choices=['pgd', 'auto'])
...
def main(args):
    ...
    # Load data
    cache_file = args.data_dir / f'imagenette.{args.data_skip}.npz'
    if not cache_file.exists():
        dataset = ImageNet(args.data_dir / 'val', transform=ImageNetteDataModule.transform_test, skip=args.data_skip)
    ...
    # Load model
    weight_path = IMAGENETTE_MODELS.get(args.load, args.load)
    model = ImageNetteResNet.load_from_checkpoint(weight_path)
